...
What is left before `how_to/001` runs is **`#asm`**, which M9 owns: `print` reaches `Default_Allocator`, whose atomics and `Runtime_Support`'s `debug_break` are `#asm` blocks. Everything else that program needs compiles. 43 of the 56 `how_to` programs now report nothing but `#asm`; the rest want M8's metaprogram (`400`, `495`, `600`, `630`) or one of the narrow gaps in `docs/spec.md` §10. A construct a later milestone owns is reported as an error *naming that milestone*, never mis-compiled; `crates/ir/tests/lower.rs` asserts that over every `how_to` program.
