...
**Every milestone is closed, and no diagnostic names one.** A construct the compiler cannot build says what the construct is — `orangejuice could not generate code for this conversion.`, `orangejuice has no encoding for the '#asm' instruction 'x'.` — rather than deferring it to a milestone number, and the options that used to print one all act: `-add` and `-run` are `add_build_string` of the text **C§2.1** wraps them in, `-context_size` is what `#Context` is padded out to, `-ps5` is `-plug ps5_build`, and `-x64` and `-debugger` are `docs/spec.md` §2's non-goals rather than unfinished work. Closing them turned up real holes, each pinned by a test: a call no overload accepts and a `return` that fills too few values are the checker's errors now, in the reference's words, and turning the first on found two bugs in `modules/` — `System.get_path_directories` and every `Socket` entry point bound to a glibc transparent union. A declaration may not take a primitive type's name. A literal in a shift takes the width that asked for it. `#import,string` is a module made out of its own text. And a `#run` sees a global's initializer rather than zeroes: a run's wrapper lowers an `__oj_global_init` of its own and calls it first, and only the run that reaches a global first assigns it. And a `$`-marked varargs parameter bakes to a `[N] T` whose elements are constants, which is what `print_vars :: ($args: .. Code)` and `Tagged_Union :: struct (value_types: .. Type)` are declared with — both modules are written the reference's way now. `Type_Info_Struct.initializer` is the generated `initializer_of(T)` rather than null, for a struct that starts as anything but zeroes, and `initializer_of` itself answers null for one that does; and every command-line option either acts or says in `docs/spec.md` §5.1 why it is a no-op.
