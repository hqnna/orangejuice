...
fn a_global_whose_value_folds_becomes_data() {
  let listing = listing_of("counter := 12;\nmain :: () { counter = 1; }\n");
...
fn a_range_loop_counts_it_and_it_index() {
  let listing = listing_of("main :: () { total := 0; for i: 1..3  total += i; }\n");
