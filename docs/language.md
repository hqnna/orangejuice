...
`#type expr` forces `expr` to be parsed as a type where the grammar would otherwise be ambiguous (needed for procedure types in value position: `SimpleProcedure :: #type () -> ();`, `PFN :: #type (a: s32) -> void #c_call;`). Procedure types in *type* position (parameter/member types) do not need `#type`: `proc: (x: int) -> int;`, `sa_handler: (sig: s32) #c_call;`.
...
The directive applies to all following declarations in the file until the next scope directive; they may alternate any number of times. The `;` is optional. `#load`ed files always get their own file scope. `#import` and `using` add names to the *currently active* scope (file or module/export). `#insert` at toplevel obeys the active scope directive. A `#placeholder` is filled by an `add_build_string` targeted at the same scope.
...
- Float arithmetic follows IEEE 754; `%`, `&`, `|`, `^`, `~` on floats are errors. `float32` and `float64` do not mix implicitly (`float32` → `float64` is allowed implicitly as a widening; `float64` → `float32` requires a cast). Integer literals convert to floats; runtime integers do not (`cast(float) n`).
...
- Comparison operators on integers, floats, pointers, enums, `bool` (`==`/`!=`), `string` (all six, byte-wise), `Type` (`==`/`!=`), procedure values (`==`/`!=`), `Code` (`==`/`!=` at compile time), and structs *only* through operator overloads. Comparing `Any` or arrays is an error. `bool != bool` is allowed.
...
3. Integer variable → wider integer that holds the full range (`u8 → u16/s16/…`, `u32 → s64`, `s32 → s64`); not `s8 → u64`, not narrowing. Runtime integers never convert implicitly to floats.
...
**Binary operators with a literal:** in a binary operation between an integer expression and a *float literal* (`level_index * 1.2`, `ms / 1000.0`, `(w - pw) / 2.`), the integer operand is converted to the literal's float type and the result has that float type. Between a runtime integer and a runtime float the operation is an error (cast explicitly). `u64 % u32` widens to `u64`. `float32 * float64` is an error.
...
- `#import` may appear in any data scope or imperative scope (procedure bodies, `#run` blocks, `#modify` blocks, macros, `#if` branches). Its names go into the scope where it appears: a file-level `#import` under `#scope_file` is visible only in that file, while under `#scope_export`/`#scope_module` the imported names are visible throughout the module (they are never re-exported to importers of this module). Bare `#import "X"` inside a procedure that would add overloads to an already-sealed set could error historically; named imports avoid this.
