...
| M5 | IR + LLVM codegen + link | **done.** `oj build` and `oj run` turn a typechecked program into a native executable that runs, through `oj-ir` → `oj-codegen` (inkwell/LLVM 19) → `oj-link` (the platform C driver); `oj dump ir` and `oj dump asm [--llvm]` print the stages. 16 programs covering procedures and overloads, locals and globals, structs with member defaults, fixed arrays and views, strings, enums, pointers, `#foreign` procedures and their libraries, `if`/`while`/`for`/`case`/`break`/`continue`/`defer`, several return values, casts and implicit conversions are compiled *and executed*, with their stdout asserted (`crates/driver/tests/build.rs`). Lowering starts at `main` and follows calls, so only reachable code reaches the back end (**L§11.6**); every one of the 56 `how_to` programs either lowers or reports an error naming the milestone that owns what stopped it, rather than crashing, hanging or mis-compiling. What is *deferred* rather than done: 55 of those 56 do stop, almost always at `print` — the acceptance that used to sit on this row, "`how_to/001` produces a running executable", belongs to M7, since `print` is a polymorphic call over `..Any` (M7) whose formatting runs `#run`s (M6, since done). The other deferral is real but narrow: an aggregate passed *by value* across a `#c_call` boundary needs SysV classification (M9) and is reported rather than mis-called |
...
| M7 | Polymorphism, macros, for_expansion, `#modify`, `#caller_code`, `#caller_location`, quick lambdas, bakes | `how_to/100–120, 170, 730`, `Hash_Table`, `Bucket_Array`, `Math`; and **`how_to/001` builds and runs** — the first program to reach `Basic.print`, which needs this milestone's polymorphic `..Any` call. M6 built the type table that call reads, so what is left is the call itself |
| M8 | Metaprogram support: the mutable program (`#insert`, `add_build_string`), `Compiler` module ABI, workspaces, messages, `Default_Metaprogram` as the driver, `Check` plugin | `how_to/400–499`, `600_insert`, `examples/output_types`, `import_replacement`, `add_build_string_into_specific_scope` |
| M9 | FFI, `#library`/pkg-config, the SysV classification of aggregates passed by value across a `#c_call` boundary, `#asm`, threads, `#c_call` callbacks, POSIX/Socket/File modules | `how_to/900+`, `Thread`, `Process`, `File_Watcher`, `Socket` examples; a `#foreign` procedure taking or returning a struct by value is called correctly rather than reported |
...
- A lookup that misses in a scope holding an unfinished name-inserting construct is a *wait*, not an error (**L§4.3**): a `using` of a value, an `#insert`, an `#asm` block that names registers, an undecidable `#if`, or a failed import. Names a macro backticks into its caller are the same case from the other side and are treated as waits program-wide until macros expand (M7). What M3 therefore cannot yet report: an identifier that only a *type* would resolve (a `using`ed struct member), and one written in a procedure body that dead-code elimination would never typecheck (**L§11.6**) — the two remaining `examples/` failures are of the second kind.
...
- IR generation is **reachability-driven from the entry point** rather than run over every declaration: asking for `main` lowers what it calls, and what those call. That is the dead code elimination of **L§11.6**, and it is also what keeps the polymorphic and macro-bearing procedures of the standard modules — which the front end deliberately leaves `unknown` until M7 — out of the back end. A construct the back end cannot lower yet is an error naming the milestone that owns it, never silently wrong code.
- The **calling convention** is one function (`oj_ir::abi_of`) that a call site and a body both build from, so the two cannot disagree. Every non-`#c_call`, non-`#no_context` procedure takes a hidden trailing `*#Context` (**L§10.1**); an argument that does not fit a register is passed as a pointer to a caller-owned copy; the first return value comes back in registers when it is scalar, and every other return is written through storage the caller passed (**L§7.2**). A `#foreign` header is `#c_call` and contextless. What is *not* implemented is the SysV classification of an aggregate passed by value across a `#c_call` boundary: that is reported as milestone M9 rather than mis-called.
...
- The C runtime's `main` is **generated by orangejuice**, and the program's own entry point takes `__program_main` — the name the reference gives it (**C§13**). The generated `main` hands the program a zeroed `#Context` and returns 0. `Runtime_Support.__system_entry_point`, which would normally own that symbol, sets temporary storage up through `initializer_of`, which is not generated yet.
...
- Compile-time execution is **driven by the checker**, not by a scheduler: asking for a `#run`'s value builds and runs it then and there, and the answer is memoized per node, so a run written once executes once. Independent runs therefore happen in the order the checker reaches them rather than in parallel, and `#run,stallable` is not scheduled differently. A `#run` written inside a polymorphic body or a macro does not run at all until M7 instantiates one (**L§12.1**); a `#run` inside a procedure nothing calls *does* run, where the reference's dead code elimination would have skipped it.
- The **type table is per lowering**, not per compilation: the executable has one image and each compile-time module has its own, so a `Type` value does not survive from a `#run` into the program that used its result. Within one image a record is laid out once, which is what makes `type_info(T) == type_info(T)` and a `Type` comparison work.
- **Linking goes through the platform C driver** (`cc`/`clang`, located by the `cc` crate) rather than driving `ld.lld` with explicit `crt1.o`/`crti.o`/`--dynamic-linker` the way the reference does (**C§11**). The driver already knows where those live; what orangejuice builds is the part that is its own — the object list, `#library` resolution, `-rpath '$ORIGIN'`, `-export-dynamic` and `--build-id`. A `#library,system "libc"` names a *file*, so the `lib` prefix comes off before `-l`.
- A `for` loop's `it` and `it_index` take their types from the loop rather than from a declaration, since a loop that does not name them declares them with no node to hang a type on. Over a range and over the three array kinds those types are known; over anything else the loop needs a `for_expansion`, which decides both names' types and is M7.
