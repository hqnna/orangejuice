...
- A type the front end cannot determine yet — a polymorphic struct's instantiation, a macro expansion, a `#run` result — is `unknown` rather than an error, and everything computed from it stays `unknown`. This is what keeps M4 from reporting the milestones it does not have.
