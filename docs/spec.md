...
| M5 | IR + LLVM codegen + link | **done.** `oj build` and `oj run` turn a typechecked program into a native executable that runs, through `oj-ir` → `oj-codegen` (inkwell/LLVM 19) → `oj-link` (the platform C driver); `oj dump ir` and `oj dump asm [--llvm]` print the stages. 16 programs covering procedures and overloads, locals and globals, structs with member defaults, fixed arrays and views, strings, enums, pointers, `#foreign` procedures and their libraries, `if`/`while`/`for`/`case`/`break`/`continue`/`defer`, several return values, casts and implicit conversions are compiled *and executed*, with their stdout asserted (`crates/driver/tests/build.rs`). Lowering starts at `main` and follows calls, so only reachable code reaches the back end (**L§11.6**); every one of the 56 `how_to` programs either lowers or reports an error naming the milestone that owns what stopped it, rather than crashing, hanging or mis-compiling. What is *deferred* rather than done: 55 of those 56 do stop, almost always at `print` — the acceptance that used to sit on this row, "`how_to/001` produces a running executable", belongs to M7, since `print` is a polymorphic call over `..Any` (M7) whose formatting runs `#run`s (M6). The other deferral is real but narrow: an aggregate passed *by value* across a `#c_call` boundary needs SysV classification (M9) and is reported rather than mis-called |
| M6 | ORC JIT compile-time execution: `#run`, `#assert`, `#if #run`, `#modify`, `#insert -> string`, data segments, `#no_reset`; the type table materialized, so `Any` and `type_info` have something to point at | `how_to/500–560`, `Basic` module compiles with its `#run`s |
| M7 | Polymorphism, macros, for_expansion, `#caller_code`, quick lambdas, bakes | `how_to/100–120, 170, 730`, `Hash_Table`, `Bucket_Array`, `Math`; and **`how_to/001` builds and runs** — the first program to reach `Basic.print`, which needs this milestone's polymorphic `..Any` call *and* M6's `#run`s, which is why M5 could not deliver it |
