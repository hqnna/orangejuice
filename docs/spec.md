...
| M6 | ORC JIT compile-time execution: `#run`, `#assert`, `#if #run`, `#modify`, `#insert -> string`, data segments, `#no_reset`; the type table materialized, so `Any` and `type_info` have something to point at | **done.** A `#run` is part of typechecking: `oj-sema` works out what the run produces and asks `oj-jit`'s engine for the value, which lowers the run into a program of its own, compiles it into an ORC `LLJIT` and calls it. Globals live in `oj-runtime`'s data segments, so two runs share one `x`; `#no_reset` is the one whose bytes reach the executable, everything else is reset (**L§12.3**). `#assert` reports the reference's wording; `#compile_time` folds per back end; a `#run` inside a polymorphic body waits for its instantiation. The type table is materialized as one self-referential image laid out against the program's own Preload, so `type_info(T)`, `Any`, and a `Type` value at runtime are all addresses into it. 21 programs in `crates/driver/tests/build.rs` are compiled *and executed* for this row alone. Over the 56 `how_to` programs nothing is left that names M6. What is *deferred* rather than done: `#insert` and `#modify`, which add declarations to a scope the front end has already built — that is the mutable program a metaprogram works on, and it moves to M8 with the rest of it; a file-scope `#if #run`, since the scope tree is built before the checker exists; and a crash inside compile-time code, which takes the compiler down with it rather than failing the workspace |
| M7 | Polymorphism, macros, for_expansion, `#caller_code`, `#caller_location`, quick lambdas, bakes | `how_to/100–120, 170, 730`, `Hash_Table`, `Bucket_Array`, `Math`; and **`how_to/001` builds and runs** — the first program to reach `Basic.print`, which needs this milestone's polymorphic `..Any` call. M6 built the type table that call reads, so what is left is the call itself |
| M8 | Metaprogram support: the mutable program (`#insert`, `#modify`, `add_build_string`), `Compiler` module ABI, workspaces, messages, `Default_Metaprogram` as the driver, `Check` plugin | `how_to/400–499`, `600_insert`, `examples/output_types`, `import_replacement`, `add_build_string_into_specific_scope` |
