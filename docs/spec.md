...
| M4 | Types & layout, constants, Match, casts, overloads, structs/enums/variants, Preload types | **done.** `oj dump types --file-only` types all 702 vendor files with no diagnostic of their own, and all 56 top-level `how_to` programs type whole with none either; the 144 standard-module struct sizes reachable from `how_to/001` match `size_of` measured with the reference compiler. What is *deferred* rather than done: a polymorphic instantiation, a macro expansion and a `#run` result are `unknown` types, which the checker never reports against, so the calls and bodies that need them wait for M6 and M7 |
...
- Bodies are typechecked per procedure: declarations, assignments, returns and conditions are Matched against what they were declared as. A polymorphic procedure's body and a macro's are left to their instantiations (M7), and operator overloading is not resolved yet, so a struct is allowed a truth value it may not have (**L§5.9**).
