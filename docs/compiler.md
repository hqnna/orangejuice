...
Two places where the shipped `Jai_Lexer` module disagrees with the compiler it was ported from, established by lexing the whole vendor tree with both and by evaluating the literals with `nix run ./vendor/jai` (orangejuice follows the compiler, not the module):

- **Exponents.** `make_number` binds `c8 := cast,no_check(u8) c` before eating the `e`, then reuses that stale `c8` after re-reading `c` (module lines 1041 and 1047), so `1.5e-3` lexes as `1.5` and `1.5e3` raises "'e' in a float literal must be followed by + or - or a numerical digit.". The compiler accepts both; 11 files in the vendor tree contain such literals (`Math/cephes.jai`'s `DP1`, `RadixSort.jai`'s `1.5e+4`, the `d3d1x` `FLT_MAX` constants, ...). The bug is upstream and reproducible without orangejuice: the module's own `examples/lex_all_files.jai`, built and run with the reference compiler, dies on the distribution's own `modules/Basic/tests.jai` after 151 files. Reading `c` afresh at both sites fixes it, and with that patch applied to a scratch copy of the module orangejuice's token stream is identical on all 702 files rather than 691 — but `vendor/` is the unmodified reference (§1 of the project spec), so the patch stays out of the tree. It follows that when the examples suite runs `lex_all_files`, exiting 1 at `Basic/tests.jai` is the *correct* result: succeeding there would mean we had mis-compiled the module.
