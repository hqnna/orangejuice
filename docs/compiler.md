...
- **Exponents.** `make_number` reuses a stale character variable in its exponent branch, so `1.5e-3` lexes as `1.5` and `1.5e3` raises "'e' in a float literal must be followed by + or - or a numerical digit.". The compiler accepts both; 11 files in the vendor tree contain such literals (`Math/cephes.jai`'s `DP1`, `RadixSort.jai`'s `1.5e+4`, the `d3d1x` `FLT_MAX` constants, ...).
